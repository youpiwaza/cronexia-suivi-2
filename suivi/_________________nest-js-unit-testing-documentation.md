...
// LOCAL NOTE: both packages (Jest & supertest) are already installed, on their latest version

// LOCAL NOTE: Backend: reference file already SOMEWHAT implemented cronexia-gta-back\app\src\resources\functions\workflow-resource\resources-submit-workflow-resource.spec.ts
...
```bash
# LOCAL NOTE: package already installed with the latest version
